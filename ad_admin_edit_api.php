...
$sql = sprintf("UPDATE `admin` SET %s WHERE sid=? AND password=SHA(?) ", implode(',', $fields));
...
$_SESSION['admin'] = $pdo->query("SELECT * FROM admin WHERE sid=". intval($_SESSION['admin']['sid']))
->fetch();
}
